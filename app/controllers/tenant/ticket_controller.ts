...
import Ticket from '#models/tenant/ticket'
...
export default class TicketController extends BaseController {
  ...
  async show({ request, response }: HttpContext) {
    try {
      const DQ = await Ticket.query().where('id', request.param('id')).first()
  ...
  async create({ auth, request, response }: HttpContext) {
    try {
      ...
      const DE = await Ticket.findBy('name', request.body().name)
  ...
  async update({ auth, request, response }: HttpContext) {
    try {
      const currentUser = auth.user!
      const DQ = await Ticket.findBy('id', request.param('id'))
      if (!DQ) {
        return response.notFound({
          code: 400,
          message: 'Data does not exists!',
